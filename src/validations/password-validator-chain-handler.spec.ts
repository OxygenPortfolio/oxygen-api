...
describe('PasswordValidatorChainHandler', () => {
	it('Should throw if password is too short', () => {
		...
		expect(() => { sut.handle(data) }).toThrow(new InvalidParamError('password must be at least 8 characters long'))
	})
	...
	it('Should throw if password is not provided', () => {
		...
		expect(() => { sut.handle(data) }).toThrow(new MissingParamError('password'))
	})
