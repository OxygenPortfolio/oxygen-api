...
describe('UsernameValidatorChainHandler', () => {
	it('Should throw if username is too short', () => {
		...
		expect(() => { sut.handle(data) }).toThrow(new InvalidParamError('username must be at least 3 characters long'))
	})
	...
	it('Should throw if username is too long', () => {
		...
		expect(() => { sut.handle(data) }).toThrow(new InvalidParamError('username must be at most 24 characters long'))
	})
	...
	it('Should throw if username is not provided', () => {
		...
		expect(() => { sut.handle(data) }).toThrow(new MissingParamError('username'))
	})
